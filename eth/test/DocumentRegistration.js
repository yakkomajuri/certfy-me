...
contract('DocumentRegistration', (accounts) => {
  ...
  // 'DocumentRegistration' regularly interacts with 'FeePool', so this initialization is essential
  ...
  it('should register a document', async () => {
    ...
    await docReg.setPoolContract(feePool.address, { from: accounts[0] });
  ...
  it('should register a multisig document', async () => {
    const docReg = await DocumentRegistration.deployed();
    await CertfyToken.deployed();
    var feePool = await FeePool.deployed();
